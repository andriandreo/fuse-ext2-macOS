...
**Fuse for macOS** can be installed via [homebrew](http://brew.sh):
...
[OSXfuse](https://osxfuse.github.io), install *via* `brew install --cask macfuse`
...
In macOS, with a `brew` install, the e2fsprogs DOES NOT live in /opt/gnu/bin and /opt/gnu/sbin. fuse-ext2 is in /usr/local/bin.

```shell
cd
dd if=/dev/zero of=/tmp/test-fs.ext4 bs=1024 count=102400  
$(brew --prefix e2fsprogs)/sbin/mkfs.ext4 /tmp/test-fs.ext4
